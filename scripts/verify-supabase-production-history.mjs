...
const addendum = JSON.parse(await readFile('supabase/production-history-addendum-20260830.json', 'utf8'));
const entries = [...baseline.entries, ...addendum.entries];
const failures = [];
let exactMatches = 0;
let terminalLfMatches = 0;
let scopedSemanticMatches = 0;

if (baseline.projectRef !== addendum.projectRef) {
  throw new Error(`Production history project mismatch: baseline=${baseline.projectRef} addendum=${addendum.projectRef}`);
}

const versions = entries.map((entry) => entry.version);
if (new Set(versions).size !== versions.length) {
  throw new Error('Duplicate production migration version across baseline/addendum.');
}
...
console.log(
  `Verified ${entries.length} canonical production migration files against the captured production ledger: ` +
