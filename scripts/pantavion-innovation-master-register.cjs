...
const families = [
  ['AI_ORCHESTRATION', /pantaai|personal ai|prime brain|specialist brain|agent|kernel|orchestrat|router/i],
  ['DYNAMIC_APP_SERVICE_CREATION', /produce applications|programs|workflows|services|app service engine|build factory|foundry|blueprint/i],
  ['INVENTION_EVOLUTION', /invention|discover.*research.*prototype|simulate|benchmark|improve|self.?evolv|evolution/i],
  ['PRODUCT_TECH_ABSORPTION', /product absorption|alternate provider|open standard|self-hosted|pantavion-native|replace.*provider/i],
  ...
  ['MARKET_WORK_BUSINESS', /marketplace|work|business|job|service|income|commerce|ads/i],
  ['LEARNING_KNOWLEDGE', /learning|education|knowledge|research|library/i],
  ['MEDIA_CREATION', /music|media|studio|creator|image|audio|video generation/i],
];

function classifyFamily(text) {
  const matches = families.filter(([, re]) => re.test(text)).map(([name]) => name);
...
if (corpus && Array.isArray(corpus.records)) {
  for (const r of corpus.records) {
    ...
    sourceRows.push({
      sourceKind: 'canonical-knowledge-v2',
      sourceId: r.id,
      ...
      text,
      existingModule: r.classification?.module || null,
      existingReviewStatus: r.reviewStatus || null,
    });
  }
}
for (const [kind, file, json] of [
  ...
  for (const f of json.findings) {
    ...
    sourceRows.push({
      sourceKind: kind,
      sourceId: `${kind}:${f.file || 'unknown'}:${f.line || 0}`,
      sourceFile: f.file || null,
      sourceLine: f.line || null,
      sourceFamily: kind,
      text,
      existingModule: null,
...
for (const row of sourceRows) {
  ...
  if (exact.has(fingerprint)) {
    exact.get(fingerprint).evidence.push(row);
    continue;
  }
  const record = {
    ...
    evidence: [row],
    nextAction: 'Semantic atomization + prior-art comparison required before any novelty claim.',
  };
...
candidates.sort((a, b) => {
  const fa = a.families.join(',');
  const fb = b.families.join(',');
  const f = fa.localeCompare(fb);
  return f || a.innovationId.localeCompare(b.innovationId);
});

const familyCounts = {};
for (const c of candidates) {
  for (const family of c.families) familyCounts[family] = (familyCounts[family] || 0) + 1;
}
const manifest = {
  id: 'pantavion_innovation_master_register_v1',
  generatedAt: new Date().toISOString(),
  truthRule: 'This register extracts evidence-backed innovation candidates. It does NOT declare novelty, world-first status, patentability, production readiness, or semantic uniqueness. Those require separate review and prior-art evidence.',
  inputs: {
  ...
  totals: {
    ...
    exactDeduplicatedCandidates: candidates.length,
    familyCounts,
  },
  requiredNextStages: [
...
const rows = [['innovation_id','families','evidence_status','novelty_status','source_count','source_files','atomic_mechanism']];
for (const c of candidates) {
  rows.push([
    c.innovationId,
    c.families.join('|'),
    ...
    c.noveltyStatus,
    c.evidence.length,
    [...new Set(c.evidence.map(e => e.sourceFile).filter(Boolean))].join('|'),
    c.atomicMechanism,
  ]);
}
