...
begin
  if not exists (
    ...
    alter table public.durable_executions
      add constraint durable_executions_lease_token_nonnegative
      check (lease_token >= 0);
  end if;
...
begin
  ...
  if p_lease_seconds is null or p_lease_seconds < 5 or p_lease_seconds > 300 then
    raise exception 'lease_seconds_out_of_bounds';
  end if;
  ...
  where d.execution_id = p_execution_id
    ...
    and (
      (
        d.status in ('queued', 'planned')
        and d.status = any(coalesce(p_expected_statuses, array['queued','planned']::text[]))
      )
      or (
        d.status = 'running'
  ...
  returning jsonb_build_object(
    ...
    'ownerId', d.lease_owner,
    'fencingToken', d.lease_token,
    'leaseExpiresAt', d.lease_expires_at,
    'heartbeatAt', d.lease_heartbeat_at,
    'attempt', d.attempt
  ) into v_result;
...
begin
  ...
  if p_succeeded is null then
    raise exception 'finish_outcome_required';
  end if;
...
-- These RPCs are control-plane only. Browser roles must never claim or mutate worker leases.
