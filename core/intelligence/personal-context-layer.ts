...
export function compilePersonalContext(input: PersonalContextInput): CompiledPersonalContext {
  ...
  if (hasScope(input.consent, "goals")) context.goals = uniqueStrings(input.goals, 20);
  if (hasScope(input.consent, "accessibility")) context.accessibility = uniqueStrings(input.accessibility, 20);
  ...
  if (hasScope(input.consent, "workflow_context") && input.workflowContext) {
    context.workflowContext = Object.fromEntries(Object.entries(input.workflowContext).slice(0, 30));
  }
  ...
  if (input.consent.allowAgentUse) {
    for (const [agent, requiredScopes] of Object.entries(AGENT_SCOPE_REQUIREMENTS) as [PersonalContextAgentKind, PersonalContextScope[]][]) {
      if (agent !== "prime" && requiredScopes.some((scope) => appliedScopes.includes(scope))) {
...
export function routePersonalContextToAgent(
  ...
  if (!compiled.allowedAgents.includes(requestedAgent)) {
    return {
      ...
      reason: "agent_not_allowed_by_owner_consent_or_context",
      context: {},
    };
  }
  ...
  return {
    ...
    agent: requestedAgent,
    authority: "temporary-scoped-read" as const,
