  ...
  return (
    <main className="min-h-screen bg-[#f4f8fc] px-4 py-8 text-slate-950">
      <section className="mx-auto max-w-3xl rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <p className="mt-3 text-slate-600">Το Social δεν θα ρίχνει πλέον ολόκληρη τη σελίδα όταν μία υπηρεσία ή migration λείπει. Η επιφάνεια παραμένει διαθέσιμη και δηλώνει καθαρά τι δεν είναι ακόμη συνδεδεμένο.</p>
        ...
        <div className="mt-5 flex flex-wrap gap-2">
          <Link href="/people" className="rounded-full bg-[#2467aa] px-4 py-2 text-sm font-black text-white no-underline">Άνθρωποι</Link>
          <Link href="/profile" className="rounded-full border border-slate-200 px-4 py-2 text-sm font-black text-slate-700 no-underline">Προφίλ</Link>
        </div>
      </section>
    </main>
  );
...
export default async function SocialPage() {
  try {
    ...
    const [profileResult, postsResult, mediaResult] = await Promise.allSettled([
      ...
      supabase.from("social_posts").select("id,author_id,body,visibility,context,location_label,created_at,social_reactions(user_id,reaction),social_comments(id,author_id,body,created_at)").order("created_at", { ascending: false }).limit(50),
