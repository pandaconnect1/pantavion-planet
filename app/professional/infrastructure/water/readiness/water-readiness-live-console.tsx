...
type LanguageCode = "el" | "en" | "tr" | "ar" | "fr" | "es" | "ru" | "de";
...
type Copy = {
  ...
  safe2: string;
  safe3: string;
  cards: { status: string; title: string; text: string }[];
...
const englishCopy: Copy = {
  ...
  languagePromise:
    "Pantavion supports user-selected language. Roadmap: 250 languages first, then progressive support for 7200 natural dialects.",
  eyebrow: "Pantavion Professional Infrastructure",
  ...
  expected: "Expected",
  jsonError: "The response was not JSON. Possible protection/auth or HTML error page.",
  ...
  cards: [
    {
      ...
      text: "The complete master remains protected and is not exposed as public geodata.",
    },
    {
      status: "Blocked",
      title: "Browser loading",
      text: "The browser does not receive the raw/full network.",
    },
    {
      status: "Contract ready",
      title: "Target viewport",
      text: "The requested area can come from location, address search, pan/zoom, or admin selection.",
    },
    {
      status: "Disambiguation required",
      title: "Repeated street names",
      text: "Repeated streets require candidate selection before bbox creation.",
    },
  ],
  requiredItems: [
...
const greekCopy: Copy = {
  ...englishCopy,
  languageLabel: "Γλώσσα",
  languagePromise:
    "Το Pantavion στηρίζει γλώσσα επιλογής χρήστη. Πρώτος στόχος: 250 γλώσσες. Μετά, σταδιακή επέκταση προς 7200 φυσικές διαλέκτους.",
  ...
  cards: [
    {
      status: "Προστατευμένο",
      title: "Πλήρες master δίκτυο",
      text: "Το πλήρες master μένει προστατευμένο και δεν εκτίθεται ως public geodata.",
    },
    {
      status: "Μπλοκαρισμένο",
      title: "Φόρτωση στον browser",
      text: "Ο browser δεν παίρνει raw/full δίκτυο.",
    },
    {
      status: "Contract ready",
      title: "Target viewport",
      text: "Η περιοχή μπορεί να προκύψει από θέση, αναζήτηση, pan/zoom ή admin selection.",
    },
    {
      status: "Απαιτείται διαχωρισμός",
      title: "Ίδιες οδοί σε πολλές περιοχές",
      text: "Ίδιες οδοί απαιτούν candidate selection πριν δημιουργηθεί bbox.",
    },
  ],
  requiredItems: [
...
const copyByLanguage: Record<LanguageCode, Copy> = {
  el: greekCopy,
  en: englishCopy,
  tr: {
    ...englishCopy,
    languageLabel: "Dil",
    languagePromise:
      "Pantavion kullanıcı seçili dili destekler. Hedef: önce 250 dil, sonra 7200 doğal lehçe.",
  ...
  ar: {
    ...
    languageLabel: "اللغة",
    languagePromise:
      "يدعم Pantavion لغة يختارها المستخدم. الهدف: 250 لغة أولاً ثم 7200 لهجة طبيعية تدريجياً.",
  ...
  es: {
    ...
    languagePromise:
      "Pantavion admite el idioma elegido por el usuario. Objetivo: 250 idiomas y luego 7200 dialectos naturales.",
    subtitle: "Centro de control de preparación en vivo — no una vitrina estática.",
    runLiveChecks: "Ejecutar control en vivo",
  },
  ru: {
    ...englishCopy,
    languageLabel: "Язык",
  ...
  de: {
    ...
    languagePromise:
      "Pantavion unterstützt die vom Nutzer gewählte Sprache. Ziel: 250 Sprachen, danach 7200 natürliche Dialekte.",
    subtitle: "Live-Bereitschaftszentrale — keine statische Anzeige.",
    runLiveChecks: "Live-Prüfung ausführen",
  },
};
...
async function loadEndpoint(endpoint: EndpointState, copy: Copy): Promise<EndpointState> {
  try {
...
export default function WaterReadinessLiveConsole() {
  ...
  const copy = copyByLanguage[language] ?? greekCopy;
        ...
        <header className="rounded-[2rem] border border-[#d8b35a]/30 bg-[#0c1830] p-7 shadow-2xl">
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            <button
              ...
              className="rounded-2xl border border-[#d8b35a]/50 bg-[#d8b35a]/15 px-5 py-3 text-sm font-bold text-[#ffe8a3] transition hover:bg-[#d8b35a]/25"
            >
              {copy.runLiveChecks}
            </button>
            ...
            <button
              type="button"
              onClick={copyPresentationMessage}
            ...
            <button
              type="button"
              onClick={() => openContract(endpoints.productionReadiness)}
              className="rounded-2xl border border-white/15 bg-white/10 px-5 py-3 text-sm font-bold text-white transition hover:bg-white/15"
            >
            ...
            <button
              type="button"
              onClick={() => openContract(endpoints.servingReadiness)}
              className="rounded-2xl border border-white/15 bg-white/10 px-5 py-3 text-sm font-bold text-white transition hover:bg-white/15"
            >
              {copy.openServingApi}
            </button>
            ...
            <button
              type="button"
              onClick={() => openContract(endpoints.addressCandidates)}
              className="rounded-2xl border border-white/15 bg-white/10 px-5 py-3 text-sm font-bold text-white transition hover:bg-white/15"
            >
              {copy.openAddressApi}
            </button>
            ...
            <button
              type="button"
              onClick={() => openContract(endpoints.controlledBbox)}
              className="rounded-2xl border border-white/15 bg-white/10 px-5 py-3 text-sm font-bold text-white transition hover:bg-white/15"
            >
              {copy.openBboxApi}
            </button>
        ...
        <section className="rounded-[2rem] border border-[#d8b35a]/25 bg-[#111a2c] p-6">
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            {endpointStates.map((endpoint) => (
              <article
                key={endpoint.key}
                className={`rounded-2xl border p-4 ${statusTone(endpoint)}`}
              >
                ...
                <p className="mt-2 text-xs opacity-75">
                  {copy.expected}: {endpoint.expectedStatus}
                </p>
                <p className="mt-3 break-words text-xs leading-5 opacity-80">
                  {endpoint.error || endpoint.path}
                </p>
              </article>
            ))}
          </div>
          ...
          <article className="rounded-[2rem] border border-white/10 bg-white/[0.04] p-7">
            ...
            <dl className="mt-5 grid gap-4">
              <div className="flex items-center justify-between rounded-2xl bg-black/20 p-4">
                <dt className="text-slate-300">{copy.overallReady}</dt>
                <dd className="font-bold text-red-200">{summary.overallReady}</dd>
              </div>
              <div className="flex items-center justify-between rounded-2xl bg-black/20 p-4">
                <dt className="text-slate-300">{copy.productionActivationAllowed}</dt>
                <dd className="font-bold text-red-200">
                  {summary.productionActivationAllowed}
                </dd>
              </div>
              <div className="flex items-center justify-between rounded-2xl bg-black/20 p-4">
                <dt className="text-slate-300">{copy.dataReturned}</dt>
                <dd className="font-bold text-emerald-200">{summary.dataReturned}</dd>
              </div>
              <div className="flex items-center justify-between rounded-2xl bg-black/20 p-4">
                <dt className="text-slate-300">{copy.mayReturnRawMaster}</dt>
                <dd className="font-bold text-emerald-200">
                  {summary.mayReturnRawMaster}
                </dd>
              </div>
              <div className="flex items-center justify-between rounded-2xl bg-black/20 p-4">
                <dt className="text-slate-300">{copy.mayReturnCompleteNetwork}</dt>
                <dd className="font-bold text-emerald-200">
                  {summary.mayReturnCompleteNetwork}
                </dd>
              </div>
            </dl>
          </article>
        ...
        <section className="grid gap-6 lg:grid-cols-[0.9fr_1.1fr]">
          <article className="rounded-[2rem] border border-white/10 bg-white/[0.04] p-7">
            ...
            <div className="mt-6 grid gap-3">
              <div className="rounded-2xl border border-[#d8b35a]/20 bg-[#d8b35a]/10 p-4 text-sm leading-7 text-[#f6e4aa]">
                {copy.selectedCandidateRequired}:{" "}
                <strong>{summary.selectedCandidateRequired}</strong>
              </div>
              <div className="rounded-2xl border border-[#d8b35a]/20 bg-[#d8b35a]/10 p-4 text-sm leading-7 text-[#f6e4aa]">
                {copy.mayAutoPick}: <strong>{summary.addressAutoPick}</strong>
              </div>
              <div className="rounded-2xl border border-[#d8b35a]/20 bg-[#d8b35a]/10 p-4 text-sm leading-7 text-[#f6e4aa]">
                {copy.bboxDataReturned}: <strong>{summary.bboxDataReturned}</strong>
              </div>
            </div>
          </article>
