...
import { createFounderAgentBudgetAssessment } from "@/core/sovereign/founder-agent-budget-assessment";
...
export async function POST(request: Request) {
  ...
  try {
    return json({
      ...
      ...createFounderAgentBudgetAssessment(input),
    }, 200);
  } catch (error) {
    const detail =
      error instanceof Error && error.message.startsWith("invalid_agent_budget_assessment:")
        ? error.message
        : "invalid_agent_budget_assessment";
    return json({ ok: false, error: "invalid_agent_budget_assessment", detail }, 400);
  }
}
