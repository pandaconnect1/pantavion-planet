...
import { revalidateSovereignExecution } from "@/core/sovereign/sovereign-execution-revalidation";
...
export async function POST(request: Request) {
  ...
  try {
    return json({
      ...
      ...revalidateSovereignExecution(input),
    }, 200);
  } catch (error) {
    const detail =
      error instanceof Error && error.message.startsWith("invalid_execution_revalidation:")
        ? error.message
        : "invalid_execution_revalidation";
    return json({ ok: false, error: "invalid_execution_revalidation", detail }, 400);
  }
}
