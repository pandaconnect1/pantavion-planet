...
import { createSovereignAdmissionBundle } from "@/core/sovereign/sovereign-admission-bundle";
...
export async function POST(request: Request) {
  ...
  try {
    return json({
      ...
      ...createSovereignAdmissionBundle(input),
    }, 200);
  } catch (error) {
    const detail =
      error instanceof Error && error.message.startsWith("invalid_sovereign_admission_bundle:")
        ? error.message
        : "invalid_sovereign_admission_bundle";
    return json({ ok: false, error: "invalid_sovereign_admission_bundle", detail }, 400);
  }
}
