...
import { createFounderTechnologyAssessment } from "@/core/sovereign/founder-technology-assessment";
...
export async function POST(request: Request) {
  ...
  try {
    return json({
      ...
      ...createFounderTechnologyAssessment(input),
    }, 200);
  } catch (error) {
    const detail =
      error instanceof Error && error.message.startsWith("invalid_technology_assessment_request:")
        ? error.message
        : "invalid_technology_assessment_request";
    return json({ ok: false, error: "invalid_technology_assessment_request", detail }, 400);
  }
}
