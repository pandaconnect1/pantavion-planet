...
import {
  ...
  DEFAULT_CROSS_CULTURAL_UNDERSTANDING_POLICY,
} from "@/core/pantavion/cross-cultural-understanding-assessment";
...
export async function POST(request: Request) {
  ...
  try {
    const assessment = assessCrossCulturalUnderstanding(
      input,
      DEFAULT_CROSS_CULTURAL_UNDERSTANDING_POLICY,
    );
    return json({
      ...
      schema: "pantavion.cross-cultural-understanding-assessment.v1",
      policy: DEFAULT_CROSS_CULTURAL_UNDERSTANDING_POLICY,
      ...assessment,
    }, 200);
  } catch (error) {
    const detail = error instanceof Error ? error.message : "invalid_understanding_request";
    return json({ ok: false, error: "invalid_understanding_request", detail }, 400);
  }
}
