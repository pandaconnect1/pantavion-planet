...
export async function POST(request: Request) {
    ...
    return NextResponse.json(
      {
        ...
        message: "Multilingual speech transcription provider is not configured.",
      },
      { status: 503 },
    );
  }
  ...
  if (!(audio instanceof File)) {
    return NextResponse.json(
      { ok: false, status: "invalid_audio", message: "Audio file is required." },
      { status: 400 },
    );
  }
    ...
    return NextResponse.json(
      {
        ...
        message: audio.size > MAX_AUDIO_BYTES ? "Audio clip is too large." : "Audio clip is empty.",
      },
      { status: 400 },
    );
  }
